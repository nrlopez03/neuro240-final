...
class DQN:
    def __init__(self, params):
        ...
        layer_name = 'fc2' ; hiddens = 256 ; dim = o1_shape[1]*o1_shape[2]*o1_shape[3]
        self.o1_flat = tf.reshape(self.o1, [-1,dim],name=self.network_name + '_'+layer_name+'_input_flat')
        self.w2 = tf.Variable(tf.random_normal([dim,hiddens], stddev=0.01),name=self.network_name + '_'+layer_name+'_weights')
        self.b2 = tf.Variable(tf.constant(0.1, shape=[hiddens]),name=self.network_name + '_'+layer_name+'_biases')
        self.ip2 = tf.add(tf.matmul(self.o1_flat,self.w2),self.b2,name=self.network_name + '_'+layer_name+'_ips')
        self.o2 = tf.nn.relu(self.ip2,name=self.network_name + '_'+layer_name+'_activations')

        # Layer 3
        layer_name = 'fc3' ; hiddens = 4 ; dim = 256
        ...
        self.y = tf.add(tf.matmul(self.o2,self.w3),self.b3,name=self.network_name + '_'+layer_name+'_outputs')
